...
export class ImageUtility {
    ...
    constructor(source: string | Blob) {
        if (typeof (source) === "string") {
            ...
            this.readFile(source, data => this.url = data);
        } else {
     ...
     */
    readFile(source: Blob, dataCallback: (dataURL: string) => void) {
         ...
         */
        const handler = (e: ProgressEvent<FileReader> )=> dataCallback.call(e, e.target.result);
     ...
     */
    createImage(src: string, handler: (source: HTMLImageElement) => HTMLCanvasElement): Promise<{ canvas: HTMLCanvasElement; naturalWidth: number; naturalHeight: number }> {
     ...
     */
    resize (targetWidth: number, targetHeight?: number, preserveAspectRatio?: boolean) {
