...
namespace Carfamsoft.Model2View.Shared
{
    ...
    public static class BindingConverter
    {
        ...
        /// Formats the provided <paramref name="value"/> as a <see cref="System.String"/>.
        /// </summary>
        /// <param name="type">The type of the specified <paramref name="value"/>.</param>
        /// <param name="value">The value to format.</param>
        /// <param name="culture">
        /// The <see cref="CultureInfo"/> to use while formatting. Defaults to <see cref="CultureInfo.CurrentCulture"/>.
        /// </param>
        /// <returns>The formatted value.</returns>
        public static object FormatValue(Type type, object value, CultureInfo culture = null)
        {
            var formatter = FormatterDelegateCache.Get(type);
            return formatter.DynamicInvoke(value, culture);
        }
        ...
        private static class FormatterDelegateCache
        {
            ...
            {
                if (!_cache.TryGetValue(type, out var formatter))
                {
                    ...
                    else if (type.IsEnum)
                    {
                        ...
                        var method = _formatEnumValue ??= typeof(BindingConverter).GetMethod(nameof(FormatEnumValueCore), BindingFlags.NonPublic | BindingFlags.Static);
                        formatter = method?.MakeGenericMethod(type).CreateDelegate(typeof(BindFormatter<object>), target: null);
                    }
                    else if (Nullable.GetUnderlyingType(type) is Type innerType && innerType.IsEnum)
                    {
                        // We have to deal invoke this dynamically to work around the type constraint on Enum.TryParse.
                        var method = _formatNullableEnumValue ??= typeof(BindingConverter).GetMethod(nameof(FormatNullableEnumValueCore), BindingFlags.NonPublic | BindingFlags.Static);
                        formatter = method?.MakeGenericMethod(innerType).CreateDelegate(typeof(BindFormatter<object>), target: null);
                    }
                    else
                    {
                        formatter = MakeTypeConverterFormatter(type);
                    }

                    _cache.TryAdd(type, formatter);
                }

                return formatter;
            }

            private static BindFormatter<object> MakeTypeConverterFormatter(Type type)
            {
                ...
                string FormatWithTypeConverter(object value, CultureInfo culture)
                {
        ...
        internal static class ParserDelegateCache
        {
            ...
            public static BindParser<T> Get<T>()
            {
                if (!_cache.TryGetValue(typeof(T), out var parser))
                {
                    ...
                    {
                        // We have to deal invoke this dynamically to work around the type constraint on Enum.TryParse.
                        var method = _convertToEnum ?? (_convertToEnum = typeof(BindingConverter).GetMethod(nameof(ConvertToEnum), BindingFlags.NonPublic | BindingFlags.Static));
                    ...
                    {
                        // We have to deal invoke this dynamically to work around the type constraint on Enum.TryParse.
                        var method = _convertToNullableEnum ?? (_convertToNullableEnum = typeof(BindingConverter).GetMethod(nameof(ConvertToNullableEnum), BindingFlags.NonPublic | BindingFlags.Static));
